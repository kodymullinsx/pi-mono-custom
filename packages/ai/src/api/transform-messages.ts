...
import { formatDocumentSummary } from "../utils/document-utils.ts";
...
function downgradeAttachmentBlocks(
	...
	const pushPlaceholder = (text: string) => {
		if (previousPlaceholder !== text) result.push({ type: "text", text });
...
function downgradeUnsupportedAttachments<TApi extends Api>(messages: Message[], model: Model<TApi>): Message[] {
	...
	if (!downgradeImages && !downgradeDocuments) return messages;
...
export function transformMessages<TApi extends Api>(
	...
	const attachmentAwareMessages = downgradeUnsupportedAttachments(normalizedMessages, model);
