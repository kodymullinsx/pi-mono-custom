...
function buildInputModalities(api: Api, rawInput: string[] | undefined): Model<Api>["input"] {
		...
		if (data.google?.models) {
			for (const [modelId, model] of Object.entries(data.google.models)) {
				...
				models.push({
					...
					input: buildInputModalities("google-generative-ai", source.modalities?.input),
					cost: {
		...
		if (data["google-vertex"]?.models) {
			for (const [modelId, model] of Object.entries(data["google-vertex"].models)) {
				...
				models.push({
					...
					reasoning: source.reasoning === true,
					input: buildInputModalities("google-vertex", source.modalities?.input),
					cost: {
						input: source.cost?.input || 0,
						output: source.cost?.output || 0,
						cacheRead,
