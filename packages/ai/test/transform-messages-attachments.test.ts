...
import type { Message, Model, ToolResultMessage, UserMessage } from "../src/types.js";
...
describe("transformMessages attachment downgrade", () => {
	...
	it("does not coalesce distinct document placeholders (each retains its own filename)", () => {
		const messages: Message[] = [
			{
				role: "user",
				content: [
