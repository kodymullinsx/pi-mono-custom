...
vi.mock("../src/utils/pdf-cache.js", () => ({
	...
	sortPDFPageImageEntries: (entries: string[]) =>
		[...entries].sort((a, b) => {
			...
			if (pageA !== undefined && pageB === undefined) return -1;
			if (pageA === undefined && pageB !== undefined) return 1;
			return a.localeCompare(b);
		}),
}));

import { extractPDFPages, parsePDFPageRange, renderPdfPagesToImageBlocks } from "../src/utils/pdf.ts";
...
describe("PDF utility behavior", () => {
	...
	beforeEach(async () => {
		...
		pdfMocks.resizeImage.mockImplementation(async (bytes: Uint8Array, mimeType: string) => ({
			type: "image",
