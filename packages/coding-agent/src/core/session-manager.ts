...
import { type Message, type PromptContentBlock, type TextContent, type Usage, uuidv7 } from "@earendil-works/pi-ai";
...
function persistAttachmentBlock(
	...
	if (!existsSync(absolutePath)) writeFileSync(absolutePath, Buffer.from(block.data, "base64"));
	return {
		storage: "sidecar",
...
function buildPersistedAttachmentPlaceholder(
	...
	return { type: "text", text: name === "document" ? "[document]" : `[document: ${name}]` };
}
...
function sanitizePromptContentForPersistence(
	...
	const sanitized = content.map((block) => {
		...
		if (!sessionFile) return placeholder;
		return {
			...placeholder,
...
function serializeFileEntryForPersistence(entry: FileEntry, sessionFile?: string): string {
	if (entry.type === "session") return JSON.stringify(entry);
...
function hydratePersistedAttachmentBlock(
	...
	if (block.type !== "text" || !("_piAttachmentRef" in block) || !block._piAttachmentRef) return block;
	const ref = block._piAttachmentRef;
	if (ref.storage !== "sidecar") return { type: "text", text: block.text };
	...
	if (!existsSync(absolutePath)) {
		...
		return { type: "text", text: `[${label} missing: expected sidecar file ${absolutePath}]` };
	}
	const data = readFileSync(absolutePath).toString("base64");
	if (ref.attachmentType === "image") return { type: "image", mimeType: ref.mimeType, data };
	return { type: "document", mimeType: ref.mimeType, data, fileName: ref.fileName };
}
...
export function loadEntriesFromFile(filePath: string, options?: { hydrateAttachments?: boolean }): FileEntry[] {
	...
	return options?.hydrateAttachments
		? entries.map((entry) => hydrateEntryFromPersistence(entry, resolvedFilePath))
		: entries;
}
...
export class SessionManager {
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (existsSync(this.sessionFile)) {
			this.fileEntries = preloadedFileEntries
				? preloadedFileEntries.map((entry) => hydrateEntryFromPersistence(entry, this.sessionFile!))
				: loadEntriesFromFile(this.sessionFile, { hydrateAttachments: true });
	...
	private _rewriteFile(): void {
		...
		try {
			for (const entry of this.fileEntries) {
				writeFileSync(fd, `${serializeFileEntryForPersistence(entry, this.sessionFile)}\n`);
			}
		} finally {
			closeSync(fd);
		}
		...
		if (!this.flushed) {
			...
			try {
				for (const e of this.fileEntries) {
					writeFileSync(fd, `${serializeFileEntryForPersistence(e, this.sessionFile)}\n`);
				}
			} finally {
				closeSync(fd);
			}
	...
	static open(path: string, sessionDir?: string, cwdOverride?: string): SessionManager {
		...
		if (cwdOverride === undefined && existsSync(resolvedPath)) {
			try {
				...
				preloadedFileEntries = loadEntriesFromFile(resolvedPath, { hydrateAttachments: true });
	...
	static forkFrom(
		...
		const sourceEntries = loadEntriesFromFile(resolvedSourcePath, { hydrateAttachments: true });
