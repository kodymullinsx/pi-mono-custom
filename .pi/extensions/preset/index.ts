...
const PRESET_ORIGINAL_STATE_ENTRY = "preset-original-state";
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			...
			pi.appendEntry(PRESET_ORIGINAL_STATE_ENTRY, originalState);
		}
		...
		if (originalState) {
			if (originalState.modelProvider && originalState.modelId) {
				...
				if (model) {
					...
					if (!success) {
						ctx.ui.notify(
							`Preset restore: No API key for ${originalState.modelProvider}/${originalState.modelId}`,
							"warning",
						);
					}
				} else {
					ctx.ui.notify(
						`Preset restore: Model ${originalState.modelProvider}/${originalState.modelId} not found`,
						"warning",
					);
				}
			}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const presetEntry = entries
			.filter((e: { type: string; customType?: string }) => e.type === "custom" && e.customType === PRESET_STATE_ENTRY)
			.pop() as { data?: { name: string } } | undefined;
		const originalStateEntry = entries
			.filter(
				(e: { type: string; customType?: string }) =>
					e.type === "custom" && e.customType === PRESET_ORIGINAL_STATE_ENTRY,
			)
			.pop() as { data?: OriginalState } | undefined;
		...
		if (presetEntry?.data?.name && !presetFlag) {
			...
			if (preset) {
				...
				activePreset = preset;
				originalState = originalStateEntry?.data;
	...
	pi.on("turn_start", async () => {
		if (activePresetName) {
			pi.appendEntry(PRESET_STATE_ENTRY, { name: activePresetName });
		}
	});
}
