...
const HIDE_SUBAGENTS_ENV = "PI_HIDE_SUBAGENTS";
...
const TaskItem = Type.Object({
  ...
  role: Type.Optional(
    Type.String({
      description:
        'Optional per-task child role override. "worker" executes directly and cannot delegate further. "coordinator" may delegate further if still within depth limits.',
    }),
  ),
});
...
const SubagentParams = Type.Object({
  ...
  role: Type.Optional(
    Type.String({
      description:
        'Optional default child role override for this call. "worker" executes directly and cannot delegate further. "coordinator" enables intentional handoff and further delegation within depth limits.',
    }),
  ),
...
function parseRequestedRole(
  raw: unknown,
): AgentRole | undefined | null {
  if (raw === undefined) return undefined;
  return parseAgentRole(raw);
}

export function resolveCurrentSessionRole(
  currentDepth: number,
  rawRole: unknown,
): AgentRole {
  const parsedRole = parseAgentRole(rawRole);
...
export function resolveChildRole(
  taskRole: AgentRole | undefined,
  callRole: AgentRole | undefined,
...
export function resolveMonitorOwnerEnv(
  currentEnv: NodeJS.ProcessEnv,
  sessionManager: SessionContextSource,
  cwd: string,
): MonitorOwnerEnv {
  const inheritedOwner: MonitorOwnerEnv = {
    sessionId: normalizeNonEmptyString(
      currentEnv[PI_MONITOR_OWNER_SESSION_ID_ENV],
    ),
    sessionFile: normalizeNonEmptyString(
      currentEnv[PI_MONITOR_OWNER_SESSION_FILE_ENV],
    ),
    cwd: normalizeNonEmptyString(currentEnv[PI_MONITOR_OWNER_CWD_ENV]),
  };

  if (
    inheritedOwner.sessionId !== undefined ||
    inheritedOwner.sessionFile !== undefined ||
    inheritedOwner.cwd !== undefined
  ) {
  ...
  const sessionFile =
    typeof sessionManager.getSessionFile === "function"
      ? normalizeNonEmptyString(sessionManager.getSessionFile())
      : undefined;
  ...
  return {
    sessionId,
    sessionFile,
    cwd: normalizeNonEmptyString(cwd),
  };
}
...
function formatAgentRole(agent: AgentConfig): string {
  return agent.role ?? DEFAULT_CHILD_AGENT_ROLE;
}

function formatAgentNames(agents: AgentConfig[]): string {
  return (
    agents.map((a) => `${a.name} (${a.source}, ${formatAgentRole(a)})`).join(", ") ||
    "none"
  );
}
...
function buildWorkerPromptGuidance(): string {
  return `\n\n## Subagent role

You are running as a \`worker\` subagent. Execute the assigned task directly, using the tools you already have. The \`subagent\` tool is unavailable in worker mode, so do not try to delegate further work.`;
}

function getInheritedNonBuiltinToolNames(
  pi: ExtensionAPI,
  role: AgentRole,
): string[] {
  if (
  ...
  if (activeToolNames.size === 0) return [];

  const inheritedToolNames: string[] = [];
  const seen = new Set<string>();
  for (const tool of pi.getAllTools()) {
    if (!activeToolNames.has(tool.name)) continue;
    if (tool.sourceInfo?.source === "builtin") continue;
    if (role === DEFAULT_CHILD_AGENT_ROLE && tool.name === "subagent") continue;
    if (seen.has(tool.name)) continue;
    seen.add(tool.name);
    inheritedToolNames.push(tool.name);
  }

  return inheritedToolNames;
}

function buildCoordinatorPromptGuidance(
  ...
  const agentList = discoveredAgents
    .map((a) => `- **${a.name}** [default: ${formatAgentRole(a)}]: ${a.description}`)
    .join("\n");

  return `\n\n## Available Subagents

You are running as a \`coordinator\` and can use the \`subagent\` tool.
...
Role behavior is controlled by optional 'role':
- 'worker' (default for delegated children): child executes directly and cannot delegate further.
- 'coordinator': child may delegate further within the existing depth and cycle guards. Use this intentionally, usually together with 'fork' for true handoff work.
...
\`\`\`json
{ "agent": "agent-name", "task": "Continue this line of investigation", "mode": "fork", "role": "coordinator" }
\`\`\`

**Parallel mode** — run multiple tasks concurrently (do NOT also set agent/task):
\`\`\`json
{ "tasks": [{ "agent": "agent-name", "task": "..." }, { "agent": "other-agent", "task": "..." }], "mode": "spawn" }
\`\`\`
...
function buildDepthBlockedPromptGuidance(
  currentDepth: number,
  maxDepth: number,
): string {
  return `\n\n## Subagent role

You are running as a \`coordinator\`, but the \`subagent\` tool is unavailable because the current delegation depth has reached the configured limit (${currentDepth}/${maxDepth}). Execute the task directly without further delegation.`;
}
...
export default function (pi: ExtensionAPI) {
  ...
  const { currentDepth, maxDepth, canDelegate, ancestorAgentStack, preventCycles } =
    depthConfig;
  const rawCurrentRole = process.env[SUBAGENT_ROLE_ENV];
  if (rawCurrentRole !== undefined && parseAgentRole(rawCurrentRole) === null) {
    console.warn(
      `[pi-subagent] Ignoring invalid ${SUBAGENT_ROLE_ENV}="${rawCurrentRole}". Expected "coordinator" or "worker".`,
    );
  }
  const currentRole = resolveCurrentSessionRole(currentDepth, rawCurrentRole);
  const canUseSubagentTool =
    currentRole === DEFAULT_ROOT_AGENT_ROLE && canDelegate;
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (
      ...
      ctx.hasUI &&
      !process.env[HIDE_SUBAGENTS_ENV]
    ) {
      const list = discoveredAgents
        .map((a) => `  - ${a.name} (${a.source}, default ${formatAgentRole(a)})`)
        .join("\n");
      ctx.ui.notify(
  ...
  pi.on("before_agent_start", async (event) => {
    if (currentRole !== DEFAULT_ROOT_AGENT_ROLE) {
      return { systemPrompt: event.systemPrompt + buildWorkerPromptGuidance() };
    }
    if (!canDelegate) {
      return {
        systemPrompt:
          event.systemPrompt +
          buildDepthBlockedPromptGuidance(currentDepth, maxDepth),
      };
    }
    ...
    return {
      systemPrompt:
        event.systemPrompt +
        buildCoordinatorPromptGuidance(
          discoveredAgents,
          currentDepth,
          maxDepth,
          preventCycles,
  ...
  if (canUseSubagentTool) {
    pi.registerTool({
      ...
      description: [
        ...
        "  role: \"worker\" (default for delegated children) -> child executes directly and cannot delegate further.",
        "  role: \"coordinator\"                         -> child may delegate further within depth/cycle limits.",
        "",
        'Example single:   { agent: "writer", task: "Rewrite README.md", mode: "spawn" }',
        'Example handoff:  { agent: "planner", task: "Continue this thread", mode: "fork", role: "coordinator" }',
        'Example parallel: { tasks: [{ agent: "writer", task: "..." }, { agent: "tester", task: "..." }], mode: "spawn" }',
      ].join("\n"),
      ...
      async execute(_toolCallId, params, signal, onUpdate, ctx) {
        ...
        const sessionManager = ctx.sessionManager as SessionContextSource;

        const requestedRole = parseRequestedRole(params.role);
        if (params.role !== undefined && requestedRole === null) {
          return {
            content: [
              {
                type: "text",
                text: `Invalid role "${String(params.role)}". Expected "coordinator" or "worker".\nAvailable agents: ${formatAgentNames(agents)}`,
              },
            ],
            details: makeDetailsFactory(
        ...
        const monitorOwner = resolveMonitorOwnerEnv(
          process.env,
          sessionManager,
          ctx.cwd,
        );
...
This guard prevents self-recursion and cyclic handoffs (for example A -> B -> A).`,
        ...
        if (params.tasks && params.tasks.length > 0) {
          ...
          for (const taskItem of params.tasks) {
            const taskRequestedRole = parseRequestedRole(taskItem.role);
            if (taskItem.role !== undefined && taskRequestedRole === null) {
              return {
                content: [
                  {
                    type: "text",
                    text: `Invalid role "${String(taskItem.role)}" for task agent "${taskItem.agent}". Expected "coordinator" or "worker".\nAvailable agents: ${formatAgentNames(agents)}`,
                  },
                ],
            ...
            const agent = agents.find((a) => a.name === taskItem.agent);
            const role = resolveChildRole(
              taskRequestedRole,
              requestedRole,
              agent?.role,
            );
            resolvedTasks.push({
              ...taskItem,
              requestedRole: taskRequestedRole,
              role,
              additionalToolNames: getInheritedNonBuiltinToolNames(pi, role),
            });
          }
        ...
        if (params.agent && params.task) {
          ...
          return executeSingle(
            ...
            role,
            requestedRole,
            getInheritedNonBuiltinToolNames(pi, role),
            delegationMode,
            forkSessionSnapshotJsonl,
            agents,
            ctx.cwd,
  ...
  async function executeSingle(
    ...
    role: AgentRole,
    requestedRole: AgentRole | undefined,
    additionalToolNames: string[],
    delegationMode: DelegationMode,
      ...
      results = await mapConcurrent(
        ...
        async (t, index) => {
          const result = await runAgent({
            ...
            role: t.role,
            requestedRole: t.requestedRole,
            additionalToolNames: t.additionalToolNames,
            delegationMode,
            forkSessionSnapshotJsonl,
            parentDepth: currentDepth,
            parentAgentStack: ancestorAgentStack,
