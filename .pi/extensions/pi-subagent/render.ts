...
function normalizeAgentRole(raw: unknown): AgentRole | null {
...
function statusIcon(r: SingleResult, theme: { fg: ThemeFg }): string {
	...
	return isResultError(r) ? theme.fg("error", "[error]") : theme.fg("success", "[ok]");
}
...
export function renderCall(args: Record<string, any>, theme: { fg: ThemeFg; bold: (s: string) => string }): Text {
	...
	const requestedRole = normalizeAgentRole(args.role);
	const roleBadge = requestedRole ? theme.fg("muted", ` [${requestedRole}]`) : "";
	...
	if (args.tasks && args.tasks.length > 0) {
		...
		for (const t of args.tasks.slice(0, 3)) {
			const taskRole = normalizeAgentRole(t.role);
			const taskRoleBadge = taskRole ? theme.fg("muted", ` [${taskRole}]`) : "";
			text += `\n  ${theme.fg("accent", t.agent)}${taskRoleBadge}${theme.fg("dim", ` ${truncate(t.task, 40)}`)}`;
		}
...
function renderSingleExpanded(
	...
	const roleBadge = r.role ? `, ${r.role}` : "";

	// Header
	let header = `${icon} ${theme.fg("toolTitle", theme.bold(r.agent))}${theme.fg("muted", ` (${r.agentSource}, ${delegationMode}${roleBadge})`)}`;
...
function renderSingleCollapsed(
	...
	const roleBadge = r.role ? `, ${r.role}` : "";
	let text = `${icon} ${theme.fg("toolTitle", theme.bold(r.agent))}${theme.fg("muted", ` (${r.agentSource}, ${delegationMode}${roleBadge})`)}`;
...
function renderParallelExpanded(
	...
	for (const r of details.results) {
		...
		const displayItems = getDisplayItems(r.messages);
		const finalOutput = getFinalOutput(r.messages);
		const roleBadge = r.role ? ` ${theme.fg("muted", `[${r.role}]`)}` : "";

		container.addChild(new Spacer(1));
		container.addChild(new Text(`${theme.fg("muted", "─── ")}${theme.fg("accent", r.agent)}${roleBadge} ${rIcon}`, 0, 0));
...
function renderParallelCollapsed(
	...
	for (const r of details.results) {
		...
		const displayItems = getDisplayItems(r.messages);
		const roleBadge = r.role ? ` ${theme.fg("muted", `[${r.role}]`)}` : "";
		text += `\n\n${theme.fg("muted", "─── ")}${theme.fg("accent", r.agent)}${roleBadge} ${rIcon}`;
