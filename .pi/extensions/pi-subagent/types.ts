...
/** Delegation role for a subagent process. */
export type AgentRole = "coordinator" | "worker";

/** Default role for the current/root session when no explicit role is set. */
export const DEFAULT_ROOT_AGENT_ROLE: AgentRole = "coordinator";

/** Default role for delegated child runs when no explicit role is set. */
export const DEFAULT_CHILD_AGENT_ROLE: AgentRole = "worker";

/** Durable monitor owner metadata propagated across subagent boundaries. */
export interface MonitorOwnerEnv {
	sessionId?: string;
	sessionFile?: string;
	cwd?: string;
}

/** Parse an agent role string. Returns null for invalid values. */
export function parseAgentRole(raw: unknown): AgentRole | null {
	if (typeof raw !== "string") return null;
	const normalized = raw.trim().toLowerCase();
	if (normalized === "coordinator" || normalized === "worker") return normalized;
	return null;
}
