...
import { parseAgentRole } from "./types.js";
...
function parseAgentFile(filePath: string, source: "user" | "project"): AgentConfig | null {
	...
	if (frontmatter.role !== undefined) {
		const parsedRole = parseAgentRole(frontmatter.role);
		if (parsedRole) {
			role = parsedRole;
		} else {
			console.warn(
				`[pi-subagent] Ignoring invalid role field in "${filePath}". Expected "coordinator" or "worker".`,
			);
		}
	}
