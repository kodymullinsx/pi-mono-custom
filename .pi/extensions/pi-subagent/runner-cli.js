...
/**
 * Check whether a --append-system-prompt value looks like a file path vs inline text.
 * File paths: absolute (/...), tilde (~), explicit relative (./, ../, .\, ..\),
 * or values with a recognizable file extension.
 * Everything else is treated as inline text and forwarded as-is.
 */
function looksLikeAppendPromptPath(value) {
  return (
...
export function parseInheritedCliArgs(argv) {
  ...
  while (i < argv.length) {
    ...
    if (flagName === "--append-system-prompt") {
      const [value, skip] = getValue();
      if (value !== undefined) {
        if (looksLikeAppendPromptPath(value)) {
          alwaysProxy.push(flagName, resolvePathArg(value));
        } else {
          // Inline text fragment: pass through as-is so Pi reads it directly.
