...
This document is the syntax reference for ast-grep rule shapes. It is not the full workflow guide.

Use it when you need exact field meanings or YAML shapes. For tool choice, matcher debugging, project workflows, custom languages, or rewrite boundaries, also load the focused references that sit next to this file.
...
* `selector` (optional): Pinpoints a specific part of the parsed pattern to match when the builtin extraction heuristic is not selecting the node you want.
  ```yaml
  pattern:
...
* `strictness`: Modifies the pattern's matching algorithm (`cst`, `smart`, `ast`, `relaxed`, `signature`, `template`).
  ```yaml
  pattern:
    context: foo($BAR)
    strictness: relaxed
...
**Best Practice**: Use `stopBy` intentionally. `stopBy: end` is useful when you want exhaustive traversal, but it is not a universal default. Choose it when crossing boundaries is actually correct for the search you mean.
