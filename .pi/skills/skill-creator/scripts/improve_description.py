...
def improve_description(
    ...
    Only train-set results are passed here. Test results are intentionally
    withheld so the improvement model cannot overfit to the held-out set.
    """
    failed_triggers = [
    ...
    # Build scores summary (only train-set results are available here)
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    if len(description) > 1024:
        ...
        if len(shortened) > 1024:
            print(f"Warning: description still {len(shortened)} chars after shortening (limit 1024)", file=sys.stderr)
