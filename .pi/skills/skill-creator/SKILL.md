...
#### Chat-history distillation for skill improvements

When mining historical chats for skill patches:

1. Start from candidate spans, not whole chats.
2. Prioritize correction loops: model assumption -> user correction -> corrected output.
3. Multi-label each span by both project and target skill when useful.
4. Generate eval prompts from correction pairs, not only positive examples.
5. Keep artifact schemas versioned and separate by type. Do not mix patch candidates, eval designs, and summaries in one JSONL.

See [references/chat-history-skill-mining.md](references/chat-history-skill-mining.md) for the detailed workflow and anti-patterns.
