...
This skill documents the strategy decided on 2026-03-25, updated 2026-03-26. Read this before re-exploring the codebase or re-deriving the architecture from scratch.
...
Nothing writes until the user approves each phase.
...
- Pi extension API key events: `before_agent_start` (can return `{systemPrompt}` to inject per-turn), `tool_result`, `tool_call`, `session_start`, `turn_end`, `session_shutdown`
