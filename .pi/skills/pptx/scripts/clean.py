"""Remove unreferenced files from an unpacked PowerPoint package.
...
RESOURCE_DIRS = ["media", "embeddings", "charts", "diagrams", "tags", "drawings", "ink", "comments"]
REL_RESOURCE_DIRS = ["charts", "diagrams", "drawings", "comments"]
...
def remove_orphaned_slides(unpacked_dir: Path) -> list[str]:
    ...
    for slide_file in slides_dir.glob("slide*.xml"):
        if slide_file.name not in referenced_slides:
            ...
            slide_file.unlink()
            removed.append(str(rel_path).replace("\\", "/"))
    ...
    if trash_dir.exists() and trash_dir.is_dir():
        for file_path in trash_dir.iterdir():
            if file_path.is_file():
                removed.append(str(file_path.relative_to(unpacked_dir)).replace("\\", "/"))
                file_path.unlink()
        trash_dir.rmdir()
    return removed


def get_slide_referenced_files(unpacked_dir: Path) -> set[Path]:
...
def remove_orphaned_rels_files(unpacked_dir: Path) -> list[str]:
    ...
    for dir_name in REL_RESOURCE_DIRS:
...
def remove_orphaned_files(unpacked_dir: Path, referenced: set[Path]) -> list[str]:
    removed = []
        ...
        for file_path in theme_dir.glob("theme*.xml"):
            ...
            if rel_path not in referenced:
                ...
                if theme_rels.exists():
                    theme_rels.unlink()
                    removed.append(str(theme_rels.relative_to(unpacked_dir)).replace("\\", "/"))
    ...
    comment_authors = unpacked_dir / "ppt" / "commentAuthors.xml"
    if comment_authors.exists() and comment_authors.relative_to(unpacked_dir) not in referenced:
        comment_authors.unlink()
        removed.append(str(comment_authors.relative_to(unpacked_dir)).replace("\\", "/"))
        rels = unpacked_dir / "ppt" / "_rels" / "commentAuthors.xml.rels"
        if rels.exists():
            rels.unlink()
            removed.append(str(rels.relative_to(unpacked_dir)).replace("\\", "/"))
...
def clean_unused_files(unpacked_dir: Path) -> list[str]:
    ...
    all_removed.extend(remove_trash_directory(unpacked_dir))
